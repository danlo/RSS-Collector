    ...
    */
    self.getAll = function(callback) {
        ...
        self.getConnection().lrange(self.historyKey(), 0, -1, function(err, reply) {
            ...
            reply.forEach(function(i) { multi.hgetall(self.itemKey(i)); } );
            
            // now get all of those keys
            multi.exec(function(err, replies) {
                // pp('multi.exec', err, replies);
                if (self.debug) {
                    console.log('getAll.multi.exec got ' + replies.length + ' hashes');
                }
                ...
                if ( replies.length === 0 ) {
                    ...
                    console.log('RSSBackend.getAll.multi.exec found no replies');
                    callback(list);
                } else {
                    replies.forEach(function(obj) {
                        var msg = new message.Message(obj);
                        list.push(msg);
                        counter += 1;
                        if ( counter === replies.length ) {
                            if (self.debug) {
                                console.log('calling RSSBackend.getAll.callback(list) with list having ' + list.length + ' items');
                            }
                            callback(list);
                        }
                    }); // replies.forEach
